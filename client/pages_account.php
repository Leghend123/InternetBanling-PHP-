...
while($row = $result->fetch_assoc()){
    ...
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param('s', $row['account_id']); // Use $row['account_id'] instead of $account_id
    $stmt->execute();
    ...
    $output .='
							...
							<tr>
								<td>
								<a href="pages_details_account.php?id='.$row['account_id'].'" style="text-decoration:none; color:black">'.$row["acc_type"].' <br><span>'.$row["account_number"].'</span></a>
								</td>
								<td>
								 <a href="pages_details_account.php?id='.$row['account_id'].' " style="text-decoration:none; color:black">Current balance <br><span style="font-size:15px; color:navy; font-weight:bold">'.$formatted_balance.'</span></a>
								 </td>
								<td>
								<a href="pages_details_account.php?id='.$row['account_id'].'" style="text-decoration:none; color:black">Available balance <br> <span style="font-size:15px;color:navy; font-weight:bold "> '.$formatted_balance.'</</span>
								</a>
								</td>
								<td>
								<a href="pages_details_account.php?id='.$row['account_id'].' "><button class="btn btn-primary">Click to See A Details</button></a>
								</td>
							</tr>
